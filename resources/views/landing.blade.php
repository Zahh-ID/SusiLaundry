...
@section('content')
    ...
    <section class="relative overflow-hidden bg-gradient-to-br from-slate-900 via-slate-900 to-slate-800">
        <div class="mx-auto grid w-full max-w-6xl gap-12 px-6 py-24 lg:grid-cols-2 lg:items-center">
            <div class="space-y-6">
                <p class="inline-flex items-center gap-2 rounded-full bg-slate-800/80 px-4 py-1 text-sm font-semibold text-primary shadow-soft">
                ...
                <div class="flex flex-wrap gap-4">
                    <a href="{{ route('order.create') }}" class="rounded-2xl bg-primary px-6 py-3 font-semibold text-white shadow-lg shadow-primary/30 hover:bg-indigo-600">
                        Pesan Sekarang
                    </a>
                <a href="{{ route('tracking') }}" class="rounded-2xl border border-slate-700 px-6 py-3 font-semibold text-slate-100 hover:border-primary">
                        Cek Status Pesanan
                    </a>
                </div>
                <dl class="grid grid-cols-2 gap-6 text-sm text-slate-300 md:grid-cols-3">
                    <div>
                        <dt class="text-3xl font-bold text-white">3K+</dt>
                        <dd>Pakaian diselesaikan per bulan</dd>
                    </div>
                    <div>
                        <dt class="text-3xl font-bold text-white">24 Jam</dt>
                        <dd>Layanan express tersedia</dd>
                    </div>
                    <div>
                        <dt class="text-3xl font-bold text-white">4.9/5</dt>
                        <dd>Rating pelanggan</dd>
                    </div>
                </dl>
            </div>
            <div class="rounded-3xl bg-slate-800/80 p-6 shadow-soft">
                <div class="space-y-5">
                    <div class="rounded-2xl border border-slate-700 p-5">
                        <p class="text-xs uppercase text-slate-400">Preview Tracking</p>
                        <p class="text-3xl font-bold tracking-widest text-white">OSL-9082</p>
                        <div class="mt-6 space-y-2 text-sm text-slate-300">
                            <p>Status: <span class="font-semibold text-emerald-400">Sedang Proses</span></p>
                            <p>Paket: Express Kilat</p>
                            <p>Antar: 19.00 - 20.00</p>
                        </div>
                    </div>
                    <div class="rounded-2xl bg-primary/90 p-5 text-white">
    ...
    <section id="layanan" class="mx-auto w-full max-w-6xl px-6 py-20">
        <div class="mb-10 text-center">
            ...
            <h2 class="text-3xl font-bold text-white">Layanan laundry lengkap & transparan</h2>
            <p class="text-slate-300">Semua kebutuhan laundry keluarga, kos, hingga bisnis bisa ter-cover dengan standar kualitas sama.</p>
        </div>
        <div class="grid gap-6 md:grid-cols-3">
            @foreach ($serviceHighlights as $highlight)
                <div class="rounded-2xl border border-slate-800 bg-slate-800 p-6 shadow-soft">
    ...
    <section id="paket" class="bg-slate-900 py-20">
        <div class="mx-auto w-full max-w-6xl px-6">
            <div class="mb-10 text-center">
                <p class="text-sm font-semibold text-primary">Daftar Paket</p>
                <h2 class="text-3xl font-bold text-white">Harga jujur, bisa disesuaikan</h2>
                <p class="text-slate-300">Semua paket sudah termasuk deterjen premium, pewangi, dan setrika rapi.</p>
            </div>
            <div class="grid gap-6 md:grid-cols-3">
                @forelse ($packages as $package)
                    <div class="flex flex-col rounded-2xl border border-slate-800 bg-slate-800 p-6 shadow-soft">
                        ...
                        <p class="mt-4 flex-1 text-sm text-slate-300">{{ $package->description }}</p>
                        <a href="{{ route('order.create') }}" class="mt-6 rounded-2xl bg-primary px-4 py-2 text-center text-white">
                            Pilih Paket Ini
                        </a>
                    </div>
                @empty
                    <p class="text-center text-sm text-slate-400">Belum ada paket terdaftar.</p>
                @endforelse
            </div>
    ...
    <section id="promo" class="bg-slate-900 py-20">
        <div class="mx-auto w-full max-w-6xl px-6">
            <div class="mb-10 text-center">
                <p class="text-sm font-semibold text-primary">Promo & Diskon</p>
                <h2 class="text-3xl font-bold text-white">Penawaran spesial untukmu</h2>
                <p class="text-slate-300">Manfaatkan kode promo berikut saat membuat pesanan.</p>
            </div>
            <div class="grid gap-6 md:grid-cols-2">
                @foreach($promos as $promo)
                    <div class="flex flex-col justify-between rounded-3xl border border-slate-800 bg-slate-800 p-6 shadow-soft">
                        <div>
                            <p class="text-sm font-semibold text-primary">Promo Laundry</p>
                            <h3 class="mt-2 text-2xl font-bold text-white">{{ $promo['title'] }}</h3>
                            <p class="mt-3 text-sm text-slate-300">{{ $promo['description'] }}</p>
                        </div>
                        <div class="mt-6 flex items-center gap-3">
                            @if ($promo['badge'])
                                <span class="rounded-full bg-slate-900 px-4 py-2 text-xs font-semibold uppercase tracking-widest text-white">Kode: {{ $promo['badge'] }}</span>
                            @endif
    ...
    <section id="tentang" class="mx-auto grid w-full max-w-6xl gap-10 px-6 py-20 lg:grid-cols-2">
        <div class="rounded-3xl border border-slate-800 bg-slate-800 p-8 shadow-soft">
            <p class="text-sm font-semibold text-primary">Tentang Kami</p>
            <h2 class="text-3xl font-bold text-white">Kami laundry rumahan yang bersertifikasi</h2>
            ...
            <div class="mt-6 flex flex-col gap-3 sm:flex-row">
                <a href="{{ route('about') }}" class="rounded-2xl bg-primary px-5 py-3 text-center text-sm font-semibold text-white">Baca selengkapnya</a>
                <a href="{{ route('contact') }}" class="rounded-2xl border border-slate-700 px-5 py-3 text-center text-sm font-semibold text-slate-100 hover:border-primary hover:text-primary">Hubungi Admin</a>
            </div>
        </div>
        <div class="rounded-3xl border border-slate-800 bg-gradient-to-br from-slate-900 to-slate-800 p-8 shadow-soft">
            <p class="text-sm font-semibold text-primary">Kontak & Lokasi</p>
            <h3 class="text-2xl font-bold text-white">Siap melayani 7 hari seminggu</h3>
            <div class="mt-6 space-y-4 text-sm text-slate-300">
                <div class="rounded-2xl border border-slate-100 p-4">
                    <p class="text-xs uppercase text-slate-400">Chat WhatsApp</p>
                    <p class="text-lg font-semibold text-white">+62 812-3456-7890</p>
                </div>
                <div class="rounded-2xl border border-slate-100 p-4">
                    <p class="text-xs uppercase text-slate-400">Telepon</p>
                    <p class="text-lg font-semibold text-white">(021) 123-4567</p>
                </div>
                <div class="rounded-2xl border border-slate-100 p-4">
                    <p class="text-xs uppercase text-slate-400">Alamat</p>
                    <p class="text-sm text-slate-300">Jl. Melati No. 18, Sleman, Yogyakarta</p>
                </div>
            </div>
