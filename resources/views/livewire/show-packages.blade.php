<div class="mx-auto w-full max-w-6xl px-6 py-16">
        ...
        @foreach($packages as $package)
            <div class="flex flex-col rounded-3xl border border-slate-100 bg-white/90 p-6 shadow-soft">
                ...
                <a href="{{ route('order.create') }}"
                    class="mt-6 w-full rounded-2xl border border-primary bg-primary px-4 py-2 text-center text-white">
                    Pesan Paket Ini
                </a>
                <a href="{{ route('contact') }}"
                    class="mt-3 w-full rounded-2xl border border-slate-200 px-4 py-2 text-center text-sm font-semibold text-slate-700 hover:border-primary hover:text-primary">
                    Hubungi Admin
                </a>
            </div>
        @endforeach
    ...
    <div class="mt-8 flex flex-col gap-3 text-center sm:flex-row sm:justify-center">
        <a href="{{ route('landing') }}"
            class="rounded-2xl border border-slate-200 px-5 py-3 text-sm font-semibold text-slate-700 hover:border-primary hover:text-primary">
            Kembali
        </a>
        <a href="https://wa.me/6285645520620" target="_blank" rel="noopener"
            class="rounded-2xl border border-primary px-5 py-3 text-sm font-semibold text-primary hover:bg-primary/10">
            Chat WhatsApp Admin
        </a>
    </div>
</div>