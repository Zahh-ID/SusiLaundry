...
@section('content')
    ...
    <div class="mx-auto mt-16 w-full max-w-4xl rounded-3xl border border-slate-100 bg-white p-8 shadow-soft">
        ...
        <div class="mt-8 grid gap-6 md:grid-cols-2">
            ...
            <div class="rounded-2xl border border-slate-100 p-5">
                ...
                <p class="text-lg font-semibold text-slate-900">{{ $order->package?->package_name }} •
        ...
        @if($order->payment_method === 'qris')
            @if($order->status === 'pending_confirmation')
                <div class="mt-8 rounded-2xl border border-blue-200 bg-blue-50 p-6 text-center">
                    <p class="text-sm font-semibold text-blue-800">Menunggu Konfirmasi Laundry</p>
                    <p class="text-xs text-blue-600 mt-1">QRIS akan muncul di halaman tracking setelah pesanan diproses oleh admin.
                    </p>
                </div>
            @elseif($latestPayment)
                <div class="mt-8 rounded-2xl border border-slate-100 bg-slate-50/70 p-6 text-center">
                    ...
                    <img src="{{ $latestPayment->qris_image_url }}" alt="QRIS"
                        class="mx-auto mt-4 h-48 w-48 rounded-xl border border-white shadow-inner">
                    <p class="mt-2 text-xs text-slate-500">Kedaluwarsa pada
                        {{ optional($latestPayment->expiry_time)->translatedFormat('d M Y H:i') }}</p>
                </div>
            @endif
        @else
        ...
        <div class="mt-8 flex flex-col gap-3 md:flex-row">
            <a href="{{ route('tracking') }}"
                class="flex-1 rounded-2xl border border-primary bg-primary px-4 py-3 text-center font-semibold text-white hover:bg-indigo-600">
                Cek Status Laundry
            </a>
            <button onclick="window.print()"
                class="flex-1 rounded-2xl border border-slate-200 px-4 py-3 font-semibold text-slate-700 hover:border-primary hover:text-primary">
                Simpan / Cetak Struk
            </button>
            <a href="{{ route('landing') }}"
                class="flex-1 rounded-2xl border border-slate-200 px-4 py-3 text-center font-semibold text-slate-700 hover:border-primary hover:text-primary">
                Kembali ke Home
            </a>
        </div>
    </div>
    ...
    <div x-data="{ open: true }" x-show="open" x-cloak
        class="fixed inset-0 z-50 flex items-center justify-center bg-black/60 px-6 py-10" x-transition.opacity>
        <div class="w-full max-w-lg rounded-3xl border border-slate-100 bg-white p-8 text-center shadow-2xl"
            x-transition.scale>
            ...
            <div class="mt-6 flex flex-col gap-3 sm:flex-row">
                ...
                <a href="{{ route('tracking', ['code' => $order->order_code]) }}"
